...
function gameLoop() {
  //Game perdido após o término das vidas
  if (lives <= 0 || !gameRunning) {
    gameRunning = false;
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    drawScore();
    drawLives();
    drawGameOver("Enter para continuar");
    drawBottomHelper();
    return false;
  }
  //Game ganho após neutralizar os invaders
  if (aliveInvaders == 0) {
    gameRunning = false;
    drawGameOver("Enter para continuar");
    drawBottomHelper();
...
function moveInvaderBullets() {
  for (let i = 0; i < invaderBulletsArray.length; i++) {
    ...
    //checa se deu game over por causa de hit dos invaders
    if (
      iB.x > tankX &&
...
function helperHandler() {
  if (aliveInvaders == armyColumns * armyRows) {
    drawBottomMessage("ESPAÇO para atirar", 125);
  } else
    if (hasLifeDecreased) {
      drawBottomMessage(`HIT. Vidas sobrando: ${lives}`, 150);
      setTimeout(() => {
        hasLifeDecreased = false;
  ...
  for (let i = 0; i < armyRows; i++) {
    for (let j = 0; j < armyColumns; j++) {
      ...
      if (soldier.status == 'alive') {
        ...
        //checa se o game acabou pela colisão com os invaders
        if (soldier.y > tankY) {
          gameRunning = false;
        }
      }
...
function moveTankBullet() {
  ...
  //checa se o invader tomou um hit da bala
  for (let i = 0; i < armyRows; i++) {
    for (let j = 0; j < armyColumns; j++) {
      let soldier = armyArray[i][j];
      if (
  ...
  //checa se um invader levou um hit das balas  
...
function drawBottomHelper() {
  drawBottomMessage("ENTER para jogar", 150)
}
...
function drawStartScreen() {
  drawBlinker(function () { drawScreen__line1("Space Invaders") }, function () { drawScreen__line2("ENTER para jogar") });
}
